...
void web_output(double stock, tm *data, int s) {
	...
	printf("<html>\n");
	printf("<head>\n");
	printf("<title>Find_the_seat</title>\n");
	printf("</head>\n");
	printf("<body>\n");
	printf("<h2 align=\"center\">The price of th tencent stock is: %.2lf</h2>\n", stock);
	printf("<h2 align=\"center\">Today is: %d-%d-%d</h2>\n", data->tm_year + 1900, data->tm_mon + 1, data->tm_mday);
	printf("<h2 align=\"center\">Next time we meet at: %d row, %d column</h2>\n", i, j);
	for (int a = 1; a <= 20; ++a) {
		for (int b = 1; b <= position[a]; ++b) {
			if(a!=i||b!=j)
			printf("<img src = \"img\\desk.png\" alt = \"some_text\" width=\"100\" height=\"100\">\n");
			else printf("<img src = \"img\\deskOn.png\" alt = \"some_text\"  width=\"100\" height=\"100\">\n");
		}
		printf("</br>");
	}
	printf("</body>\n");
